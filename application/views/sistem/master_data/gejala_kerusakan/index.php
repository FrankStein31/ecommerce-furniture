...
function loadData() {
  ...
  $.ajax({
    url: '<?= site_url("Master_data/fetch_gejala_kerusakan") ?>',
    type: 'GET',
    dataType: 'json',
  ...
  if (searchValue === '') {
    ...
    filteredData = allData.filter(item => {
      return item.kode.toLowerCase().includes(searchValue) ||
             item.gejala_kerusakan.toLowerCase().includes(searchValue);
    });
  }
    ...
    paginatedData.forEach((item, index) => {
      ...
      html += `
        <tr>
          ...
          <td class="text-center"><span class="badge badge-success">${item.kode}</span></td>
          <td>${item.gejala_kerusakan}</td>
        </tr>
      `;
    });
  } else {
