...
function loadData() {
  ...
  $.ajax({
    url: '<?= site_url("Master_data/fetch_jenis_perbaikan") ?>',
    type: 'GET',
    dataType: 'json',
  ...
  if (searchValue === '') {
    ...
    filteredData = allData.filter(item => {
      return item.kode.toLowerCase().includes(searchValue) ||
             item.jenis_perbaikan.toLowerCase().includes(searchValue);
    });
  }
    ...
    paginatedData.forEach((item, index) => {
      ...
      html += `
        <tr>
          ...
          <td class="text-center"><span class="badge badge-info">${item.kode}</span></td>
