  ...
  <div class="col-12">
    <div class="card flat">
      <div class="card-header card-header-blue">
        <span class="card-title">Edit Gejala Kerusakan</span>
      </div>
      <div class="card-body">
        <form method="post" action="<?= base_url('gejala_kerusakan/edit/'.$gejala->id) ?>">
            ...
            <div class="col-md-8">
              <div class="form-group">
                ...
                <input type="text" 
                       ...
                       value="<?= set_value('nama_gejala', $gejala->nama_gejala) ?>" 
                       required>
          ...
          <div class="form-group">
            ...
            <textarea name="deskripsi" 
                      ...
                      rows="3" 
                      placeholder="Deskripsi detail gejala kerusakan (opsional)"><?= set_value('deskripsi', $gejala->deskripsi_gejala) ?></textarea>
          ...
          <div class="form-group">
            ...
            <textarea name="pertanyaan" 
                      ...
                      rows="2" 
                      placeholder="Contoh: Apakah furniture Anda mengalami gejala ini?"><?= set_value('pertanyaan', $gejala->pertanyaan) ?></textarea>
          ...
          <div class="form-group">
            ...
            <select name="jenis_perbaikan[]" 
              ...
              <?php foreach($jenis_list as $id => $nama): ?>
                <option value="<?= $id ?>" 
                        <?= in_array($id, $selected_jenis) ? 'selected' : '' ?>>
                  <?= $nama ?>
                </option>
              <?php endforeach; ?>
            </select>
          ...
          <div class="form-group">
            ...
            <select name="status" class="form-control <?= form_error('status') ? 'is-invalid' : '' ?>" required>
              <option value="1" <?= set_select('status', '1', $gejala->status == '1') ?>>Aktif</option>
              <option value="0" <?= set_select('status', '0', $gejala->status == '0') ?>>Nonaktif</option>
            </select>
        ...
        <div class="card bg-light mt-3">
          <div class="card-body">
            <small class="text-muted">
              ...
              <strong>Dibuat:</strong> <?= date('d-m-Y H:i:s', strtotime($gejala->created_at)) ?> 
              <?php if($gejala->updated_at): ?>
                | <strong>Terakhir Diupdate:</strong> <?= date('d-m-Y H:i:s', strtotime($gejala->updated_at)) ?>
              <?php endif; ?>
            </small>
          </div>
        </div>
